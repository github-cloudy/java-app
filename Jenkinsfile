pipeline {
    ...
    environment {
        DOCKER_IMAGE = '021891609956.dkr.ecr.ap-south-1.amazonaws.com/java-app'
        ...
        stage('Commit to Deploy') {
            steps {
                script {
                    ...
                    sh "sed -i 's|021891609956.dkr.ecr.ap-south-1.amazonaws.com/java-app:[0-9]*|${DOCKER_IMAGE}:${BUILD_NUMBER}|g' deployment.yaml"
                    // Set Git credentials
                    withCredentials([usernamePassword(credentialsId: 'java_repo_creds', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                        sh '''
                            ...
                            git remote set-url origin https://$GIT_USERNAME:$GIT_PASSWORD@github.com/https://github.com/github-cloudy/java-app
                            git push
                        '''
                    }
                }
